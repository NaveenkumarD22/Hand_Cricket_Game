...
def print_banner():
    print("\033[4m" + "WELCOME TO THE GAME OF HAND-CRICKET!")
    print()
    print("Game Instructions:" + "\033[0m")
...
def check_toss():
    ...
    if (toss_userchoice + toss_botchoice) % 2 == 0:
        ...
        odd_or_even = "odd"
    print()
    print("It's ",odd_or_even.upper(),"!", sep = "")
...
def outro_printbanner(winner, batter, bowler, score, chase):
    ...
    if winner == "Player":
        winner = "You"
    print("The winner of this match is ", winner, "!", sep = "")
    if winner == "None":
        print("Both of you have scored the same number of runs.")
    else:
        if score >= chase:
            print(bowler, "scored", chase-1, "run(s) in the first inning, and", batter, "successfully chased it.")
        else:
            print(bowler, "scored", chase-1, "run(s) in the first inning, but unfortunately", batter, "couldn't chase it and got out with", chase-score, "run(s) to win.")
    
    if winner == "You":
        print("CONGRATULATIONS! YOU HAVE WON THE MATCH!")
    elif winner == "Bot":
        print("UNLUCKY! THE BOT HAS WON THE MATCH...")
    else:
